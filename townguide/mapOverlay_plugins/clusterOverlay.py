...
class clusterOverlay(mapOverlayBase.mapOverlayBase):
    """
    A townguide mapOverlay renderer that draws numbered markers to show points
    of interest on the map image.  They are clustered into a single marker if
    ...
    def renderBody(self):
        """
        Draw the pois and clusters onto the specified image file.
        """
        print "clusterOverlay.render()"
        ...
        # FINISHED drawing the pois ========================

        # Draw some cluster markers
        markerSize = int(self.townguide.preferences_list['markersize']) * int(self.townguide.preferences_list['dpi'])/72
            
        for (featureNo, kluster) in enumerate(self.townguide.clusters):
            print "Drawing the cluster: ", kluster
            self.agg_cluster_brush = aggdraw.Brush("#77AABB", opacity=200)
            
            fx = kluster.centroid[1]
            fy = kluster.centroid[0]
            imx = int((fx - self.c0.x)/self.townguide.preferences_list['oscale'])
            imy = self.im.size[1] - int((fy - self.c0.y)/self.townguide.preferences_list['oscale'])

            origin_x = imx - markerSize/2
            origin_y = imy - markerSize/2
            
            clusterSize =  2 * markerSize * len(kluster.pois)
            
            self.agg_canvas.ellipse((origin_x,
                                origin_y,
                                origin_x + clusterSize,
                                origin_y - clusterSize),
                               self.agg_pen, self.agg_cluster_brush)
                

                
            self.agg_font_clusters = aggdraw.Font("white", self.townguide.fntPath, self.fntSize * 1.3 * len(kluster.pois), opacity=255)
            self.agg_canvas.text((origin_x + 0.6 * markerSize * len(kluster.pois),
                             origin_y - 1.8 * markerSize * len(kluster.pois)),
                            "%d" % len(kluster.pois),
                            self.agg_font_clusters)
            # self.agg_canvas.text((origin_x + 0.6 * markerSize,
            #                  origin_y - 1.8 * markerSize),
            #                 "%d" % len(kluster.pois),
            #                 self.agg_font_clusters)
