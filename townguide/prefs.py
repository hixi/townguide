...
class prefs(handler.ContentHandler):
    ...
    def elementtodict(self,parent):
        ...
        while child is not None:
            if (child.nodeType == xml.dom.minidom.Node.ELEMENT_NODE):
                ...
                d[child.tagName] = self.elementtodict(child)
    ...
    def loadPrefsFromString(self, xmlStr):
        """Load a Preferences XML string 'xmlStr' into memory and
        return a dictionary containing the preferences specified in the file
        Modified by Waldemar Quevedo
        """
        
        try:
            self.tags = self.xmltodict(xmlStr)
            # parser = make_parser()
            # parser.setContentHandler(self)
            # parser.parseString(xmlStr)
            # self.fname = filename
    ...
    def applyDefaults(self,defs):
        ...
        """
        for key in defs:
            print "Checking default %s : %s" % (key, defs[key])
