...
# Register unicode font that comes bundled with mapnik
...
class simpleLandscapeRenderer( ):
    """
    A townguide plugin for rendering a map in landscape mode
    ...
    def __init__(self, townguide):
        ...
        (self.page_height, self.page_width) = self.pagesize
    ...
    def laterpages_canvas_callback(self,canvas, document):
        ...
        canvas.drawString(document.leftMargin,
                          self.page_height - self.page_height * 0.1,
                          "Points of Interest"
                          )
    ...
    def render(self):
        ...
        if map_width > map_height:
            ...
            #  ----------------------------
            HORIZONTAL = True
            aspect_ratio = map_height / map_width  # if width is larger
            mapframe_width = self.page_width * 1.0 - self.page_width * 0.05
            mapframe_height = mapframe_width * aspect_ratio
            detailframe_width = self.page_width * 0.9 - self.page_width * 0.07 
            detailframe_height = self.page_height * 0.2
            mapframe_left_margin = self.page_width * 0.02
            mapframe_bottom_margin = self.page_height - document.topMargin - titleframe_height - mapframe_height
            
            # if the map height is too large
            # use a fixed height and compute the width again
            if mapframe_height > (self.page_height * 0.6):
                print "************************************************"
                print "mapframe_height too large to fit with that aspect_ratio: ", mapframe_height
                mapframe_height = self.page_height * 0.55
                aspect_ratio = map_width / map_height  # if height is larger
                mapframe_width = mapframe_height * aspect_ratio
                mapframe_bottom_margin = self.page_height - document.topMargin - titleframe_height - mapframe_height
                mapframe_left_margin = document.leftMargin
                print "************************************************"
            
            mapframe = Frame (
                mapframe_left_margin, 
                mapframe_bottom_margin,
                mapframe_width,  # document.width,
                mapframe_height + 0.2*cm, # document.height - 5*cm,
                showBoundary=None,
                leftPadding=0,
                rightPadding=0,
                topPadding=0.1*cm,
            ...
            # document.bottomMargin + detailframe_height, # + 0.5*cm - detailframe_height,
            # document.leftMargin + self.page_width * 0.9,
            detailframe = Frame(
                document.leftMargin,
                self.page_height - document.topMargin - titleframe_height - mapframe_height - detailframe_height,
                detailframe_width,
                detailframe_height,
                showBoundary=None,
                leftPadding=0,
            ...
            # mapframe_width = 11*cm
            # mapframe_height = 16*cm
            # ----------------------------
            #  Title                      |  
            #  .............. Description |
            #  ..          ..             |
            ...
            mapframe_height = self.page_height * 0.7
            mapframe_width = mapframe_height * aspect_ratio
        ...
        # print 'aspect_ratio is ', aspect_ratio, "and horizontal? ", HORIZONTAL
        # print "mapframe_width: ", mapframe_width, " and mapframe_height: ", mapframe_height
        ...
        self.townguide.drawOverviewMap(self.townguide.outdir,addFeatures=False)
